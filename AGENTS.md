...
- **Styling and Typography:** This web-only Next.js application uses Tailwind CSS utilities in `app/` and `components/`, with shared global styles and theme transitions in `styles/globals.css`. Preserve the existing Headless UI and Motion owners, approved breakpoints, responsive wrapping, accessible text scaling, and reduced-motion behavior. Do not introduce CSS Modules, BEM, `@apply` component-selector systems, selector dictionaries, or static inline appearance styles when the existing utilities work. CSS-first geometry and measurement by exception: CSS must own sizing, positioning, and motion geometry it can express through existing layout constraints, intrinsic sizing, percentages, authored inputs, and supported CSS arithmetic. Do not reconstruct those relationships through `ResizeObserver`, resize listeners, layout reads, or measurement-derived React state. Assess the complete measurement → state/style update → layout/animation feedback loop, including unnecessary rendering work, visible corrections, and repeated-layout risks. Authored asset dimensions and genuine runtime animation inputs do not authorize measuring CSS-owned output into a second layout authority. A measurement exception requires a concrete approved behavior, evidence that the supported platform cannot satisfy it declaratively, and Mapachito’s explicit approval for the smallest scoped implementation. Cleanup, batching, and passing tests do not establish necessity. Inherited styling or measurement drift is not precedent; propose a bounded correction before extending it. Do not introduce a new styling or measurement framework. You must exclusively use “Curly Double Quotes” (“ ”) and curly apostrophes (’) in all UI-facing text. Straight quotes are banned in the UI presentation layer.
