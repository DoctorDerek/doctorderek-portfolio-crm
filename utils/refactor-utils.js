...
files
  ...
  .forEach((file) => {
    ...
    if (
      content.includes("calculateAge") &&
      !file.includes("calculateAge.ts") &&
      !file.includes("CONTACTS.tsx")
    ) {
      content =
        `import { calculateAge } from "@/utils/calculateAge"\n` + content
    }
    if (
      content.includes("sortByLastName") &&
      !file.includes("sortByLastName.ts") &&
      !file.includes("CONTACTS.tsx")
    ) {
      content =
    ...
    if (file.includes("phoneBookMachine.ts")) {
      ...
      content = content.replace(
        /import CONTACTS_WITH_AGES, \{ calculateAge, sortByLastName \} from "@\/contacts\/CONTACTS"/g,
        'import CONTACTS_WITH_AGES from "@/contacts/CONTACTS"',
      )
    }
    ...
    if (file.includes("CONTACTS.tsx")) {
      content = content.replace(
        /export const calculateAge = \(\{\s*birthYear,\s*birthMonth,\s*birthDay,\s*\}\: \{\s*birthYear\?\: string\s*birthMonth\?\: string\s*birthDay\?\: string\s*\}\) \=\> \{[\s\S]*?return age\n\}\n/g,
        'import { calculateAge } from "@/utils/calculateAge"\n',
      )
      content = content.replace(
        /export const sortByLastName = \(a\: Contact, b\: Contact\) \=\> \{[\s\S]*?return aLastName.localeCompare\(bLastName\)\n\}\n/g,
        'import { sortByLastName } from "@/utils/sortByLastName"\n',
      )
